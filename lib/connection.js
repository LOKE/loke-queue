...
function Connection (url, options) {
  ...
  .then(function (client) {
    ...
    .then(function (ch) {
      return ch.assertExchange(exchange, 'topic', { durable: isDurable, autoDelete: options.autoDelete || false })
      .then(function () {
        self.emit('connect')
        return ch
      })
...
Connection.prototype._getWaitExchangeName = function (retryAfterMilliseconds) {
  ...
  .then(function (ch) {
    return ch.assertExchange(waitExchange, 'topic', { durable: isDurable, autoDelete: autoDelete })
    .then(function () {
    ...
    .then(function (ok) {
      return ch.bindQueue(ok.queue, waitExchange, '#')
      .then(function () {
        return waitExchange
      })
    })
